  ...
  const favMovies = async () => {
    try {
      const response = await axios.get(
        `https://api.themoviedb.org/3/account/${token}/favorite/movies?language=${selectLanguage}&page=1&sort_by=created_at.asc`,
        {
          headers: {
  ...
  return (
    <>
      <div className="sticky-header flex items-center justify-between mt-3 w-screen px-5">
        <p className="flex items-center text-5xl text-shadow-black text-red-600">
          <Link to="/">
            <strong>Cinephile</strong>
          </Link>
        </p>
        <div className="flex items-center border py-2 mr-5 px-4 rounded-full border-red-600 text-white">
          <input
            type="text"
      ...
      <div>
        <h1 className="text-center my-8 mt-14 font-bold mb-2 text-white text-3xl">
          Favorite Movie
        </h1>
        {movies.length === 0 && (
          <div className="flex items-center justify-center mt-12 h-full text-white text-center">
